...
public class VertiBayesCentralServerTest {
    ...
    @Test
    public void testBinning() throws JsonProcessingException {
        BayesServer station1 = new BayesServer("resources/Experiments/iris/iris_firsthalf.csv", "1");
        BayesServer station2 = new BayesServer("resources/Experiments/iris/iris_secondhalf.csv", "2");
        ...
        List<WebNode> noBinsWeb = central.buildNetwork().getNodes();
        WebBayesNetwork req = new WebBayesNetwork();
        req.setNodes(noBinsWeb);

        List<Node> noBins = WebNodeMapper.mapWebNodeToNode(central.maximumLikelyhood(req).getNodes());

        List<WebNode> bins = central.buildNetwork().getNodes();
        WebBayesNetwork reqBins = new WebBayesNetwork();

        ObjectMapper mapper = new ObjectMapper();
        System.out.println(
                mapper.writerWithDefaultPrettyPrinter().writeValueAsString(central.maximumLikelyhood(req).getNodes()));

        reqBins.setNodes(bins);
